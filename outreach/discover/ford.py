...
log = logging.getLogger("outreach.discover.ford")

# Ford Australia is protected by Akamai WAF which blocks headless browsers.
# Approach: use Google Maps search to find Ford dealers in Australia,
# then scrape from Google's results. This is a fallback approach until
# Akamai bypass is implemented (e.g. via curl_cffi or residential proxy).

SEARCH_URL = "https://www.google.com/maps/search/Ford+dealer+Australia/"

# Australian state capital coordinates for broad coverage
SEARCH_LOCATIONS = [
    ...
    {"name": "Bendigo", "lat": -36.7574, "lng": 144.2794},
    ...
    {"name": "Tamworth", "lat": -31.0833, "lng": 150.9167},
]

SCRAPE_JS = """() => {
    const results = document.querySelectorAll('a[href*="/maps/place/"]');
    return Array.from(results).map(el => {
        const text = el.closest('[role="article"]');
        const fullText = text ? text.textContent : el.textContent;
        const name = el.getAttribute('aria-label') || el.textContent.trim();
        return {name: name.trim(), text: fullText ? fullText.substring(0, 400) : ''};
    }).filter(r => r.name);
}"""
...
@register("ford")
def discover_ford(limit: int = 0) -> int:
    log.info("loading ford dealer data via google maps search (akamai bypass)")
    ...
    with sync_playwright() as p:
        ...
        for loc in SEARCH_LOCATIONS:
            try:
                url = f"https://www.google.com/maps/search/Ford+dealer/@{loc['lat']},{loc['lng']},10z"
                ...
                # Extract dealer data from search results
                results = page.evaluate("""() => {
                    ...
                    return Array.from(items).map(a => {
                        ...
                        return {name: label.trim(), href, text: text.substring(0, 500)};
                    }).filter(r => r.name && r.name.toLowerCase().includes('ford'));
                }""")
                ...
                log.warning("error searching ford in %s: %s", loc["name"], e)
    ...
    if not dealers:
        log.error("no ford dealers captured — Akamai WAF blocked, Google Maps fallback returned no results")
        return 0

    log.info("scraped %d unique ford dealers from google maps", len(dealers))
    inserted = 0
    ...
    with get_conn() as conn:
        for d in dealers:
            ...
            # Parse address from the text blob
            text = d.get("text", "")
            suburb = None
            state = None
            postcode = None
            ...
            cur = conn.execute(
                ...
                "RETURNING id",
                ("ford", name, address, suburb, state, postcode, phone, None),
            )
            if cur.fetchone():
                inserted += 1
    ...
    log.info("inserted %d new ford dealerships", inserted)
    return inserted
