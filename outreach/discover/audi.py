...
log = logging.getLogger("outreach.discover.audi")

# Audi Australia website (audi.com.au) returns 503 - "Site currently not available".
# Fallback: use Google Maps search to find Audi dealers in Australia.
# When the site is back up, the dealer API endpoint is likely:
#   https://www.audi.com.au/bin/audi/au/dealerlocator/search?brand=A&country=AU
...
@register("audi")
def discover_audi(limit: int = 0) -> int:
    log.info("loading audi dealer data via google maps search (site 503)")
    ...
    with sync_playwright() as p:
        ...
        for loc in SEARCH_LOCATIONS:
            try:
                url = f"https://www.google.com/maps/search/Audi+dealer/@{loc['lat']},{loc['lng']},10z"
                ...
                results = page.evaluate("""() => {
                    ...
                    }).filter(r => r.name && r.name.toLowerCase().includes('audi'));
                }""")
                ...
                log.warning("error searching audi in %s: %s", loc["name"], e)
    ...
    if not dealers:
        log.error("no audi dealers captured — site 503, Google Maps fallback returned no results")
        return 0

    log.info("scraped %d unique audi dealers from google maps", len(dealers))
    inserted = 0
    ...
    with get_conn() as conn:
        for d in dealers:
            ...
            cur = conn.execute(
                ...
                "RETURNING id",
                ("audi", name, address, suburb, state, postcode, phone, None),
            )
            if cur.fetchone():
                inserted += 1
    ...
    log.info("inserted %d new audi dealerships", inserted)
    return inserted
